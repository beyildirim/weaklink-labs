...
In PPE (Lab 2.2), the attacker modifies the workflow YAML. In expression injection:
...
**Splunk -- detect shell metacharacters in issue titles:**

```spl
index=git sourcetype=gitea event_type="issues"
| where match(issue_title, "[\x60\$\(\)\;\|\&]{2,}")
| table _time, repository, issue_author, issue_title, issue_number
| sort -_time
```

**Splunk -- detect outbound connections from issue-triggered workflows:**

```spl
index=cicd sourcetype=build_logs trigger_event="issues"
| where match(step_output, "(curl|wget|nc|ncat|dns|http)")
  AND NOT match(step_name, "(checkout|setup-node|setup-python)")
| table _time, repository, workflow_name, step_name, step_output, issue_number
```

**KQL -- detect injection attempts in PR branch names:**

```kql
DeviceProcessEvents
| where InitiatingProcessFileName == "bash"
| where ProcessCommandLine has_any ("$(", "`;", "&&", "||")
| where InitiatingProcessCommandLine has "actions-runner"
| join kind=inner (
    CIBuildEvents | where TriggerType in ("issues", "issue_comment", "pull_request")
) on $left.DeviceId == $right.RunnerId
| project Timestamp, ProcessCommandLine, Repository, TriggerType, EventAuthor
```
