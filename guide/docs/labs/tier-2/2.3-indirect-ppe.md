...
<div class="lab-meta">
  ...
  <span>Prerequisites: Lab 2.2</span>
</div>

After Direct PPE (Lab 2.2) became well-known, many organizations locked down their CI configuration files -- requiring code review, CODEOWNERS approval, or running the target branch's CI config instead of the PR's version. Attackers adapted.
...
**Splunk -- detect modification of CI-referenced files in PRs:**

```spl
index=git sourcetype=gitea event_type="pull_request"
| where match(files_changed, "(Makefile|scripts/|Dockerfile|\.sh$)")
| where NOT match(files_changed, "(\.github/workflows|\.gitea/workflows)")
| eval risk="CI-referenced files changed without CI config change"
| table _time, repository, author, pr_title, files_changed, risk
```

**Splunk -- detect curl/wget in Makefiles:**

```spl
index=git sourcetype=gitea event_type="push"
| where match(files_changed, "Makefile")
| where match(diff, "(curl|wget|nc|base64|python -c)")
| table _time, repository, author, commit_message, diff
```

**KQL -- detect outbound connections during make/test steps:**

```kql
DeviceNetworkEvents
| where RemoteIPType == "Public"
| where InitiatingProcessCommandLine has_any ("make", "bash scripts/", "pytest")
