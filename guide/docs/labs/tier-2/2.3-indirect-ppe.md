...
**Sigma -- CI-Referenced Files Modified in PR Without CI Config Change:**
...
*   **Known False Positives**: Build script changes are normal. Escalate when diffs add curl, wget, nc, or secret variable references.
*   **Tuning Strategy**: Parse the diff for suspicious commands. Alert only when network or secret-reading commands are added.
</details>

**Sigma -- Exfiltration Commands Added to Build Scripts:**
...
**Tuning & False Positives**
*   **Known False Positives**: Dockerfiles use curl to install packages. Check destination URLs -- internal/known vs. external.
*   **Tuning Strategy**: First-time contributors adding curl to Makefiles is high-signal.
</details>

**Sigma -- Outbound Connections During Make/Test CI Steps:**
...
**Tuning & False Positives**
*   **Known False Positives**: Integration tests calling external APIs. Create per-repo allowlists.
