...
**Splunk -- detect include:remote changes in CI config:**

```spl
index=gitlab sourcetype=gitlab_audit event_type="repository_file_change"
  file_path=".gitlab-ci.yml"
| where match(diff_content, "include:.*remote:")
| table _time, project, user, diff_content
| sort -_time
```

**Splunk -- detect variable access in MR pipelines:**

```spl
index=gitlab sourcetype=gitlab_ci_logs pipeline_source="merge_request_event"
| where match(job_output, "(TOKEN|SECRET|KEY|PASSWORD|CREDENTIAL)")
| table _time, project, pipeline_id, job_name, user, job_output
| sort -_time
```

**Splunk -- detect cross-project triggers from MR pipelines:**

```spl
index=gitlab sourcetype=gitlab_audit event_type="pipeline_trigger"
| where source_pipeline_source="merge_request_event"
| table _time, source_project, target_project, source_pipeline_id, user
| sort -_time
```

**KQL -- detect include:remote with external URLs:**

```kql
GitLabAuditEvents
| where EventType == "ci_config_change"
| where ConfigContent has "include:" and ConfigContent has "remote:"
| extend RemoteURL = extract("remote:\\s*['\"]?(https?://[^'\"\\s]+)", 1, ConfigContent)
| where RemoteURL !has "gitlab.internal"
| project Timestamp, Project, User, RemoteURL
```

**KQL -- detect MR pipeline accessing sensitive variables:**

```kql
GitLabCILogs
| where PipelineSource == "merge_request_event"
| where JobOutput has_any ("TOKEN", "SECRET", "KEY", "AWS_", "DEPLOY_")
| project Timestamp, Project, PipelineId, JobName, User
```
