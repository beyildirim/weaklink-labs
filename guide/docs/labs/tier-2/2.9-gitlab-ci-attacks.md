...
The downstream pipeline runs with the permissions and variables of the **target project**. If `ops/deploy-infra` has production deployment credentials, any pipeline in `acme-webapp` that reaches the trigger stage can access them indirectly.
