...
**Splunk -- detect marketplace image launches:**

```spl
index=cloudtrail eventName="RunInstances"
| spath output=ami_id path=requestParameters.instancesSet.items{}.imageId
| lookup approved_ami_list ami_id OUTPUT is_approved
| where isnull(is_approved) OR is_approved!="true"
| eval severity="high"
| eval mitre="T1525"
| table _time, userIdentity.arn, ami_id, sourceIPAddress, severity, mitre
```

**KQL -- detect unexpected outbound connections from new instances:**
...
) on $left.VM_s == $right.VMID
| where datetime_diff("hour", TimeGenerated, TimeGenerated1) < 1
| project TimeGenerated, VM_s, DestIP_s, DestPort_d, FlowStatus_s
```

**Suricata -- detect phone-home traffic:**

```yaml
alert http $HOME_NET any -> $EXTERNAL_NET any (
  msg:"WEAKLINK Marketplace Poisoning - Phone home to cloud-analytics.io";
  flow:established,to_server;
  http.host; content:"cloud-analytics.io";
  classtype:trojan-activity;
  sid:9100001; rev:1;
  metadata:mitre_technique T1525;
)

alert dns $HOME_NET any -> any 53 (
  msg:"WEAKLINK Marketplace Poisoning - DNS exfil to cloud-analytics.io";
  dns.query; content:"exfil.cloud-analytics.io";
  classtype:trojan-activity;
  sid:9100002; rev:1;
