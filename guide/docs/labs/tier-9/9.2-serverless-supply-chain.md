...
`requirements.txt` references internal packages (`acme-order-utils`, `acme-notification-client`) but `sam build` runs `pip install` with default index. Public PyPI queried first.
...
`sam build` runs `pip install acme-order-utils>=1.0.0`. Public PyPI has version `99.0.0`. The attacker's malicious package wins resolution and gets bundled into the Lambda deployment package.
