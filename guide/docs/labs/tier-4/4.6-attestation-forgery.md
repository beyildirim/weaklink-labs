...
**Sigma -- Attestation Created Outside CI:**

```yaml
title: Attestation Created Outside CI
id: wl-46-0dfdc8aa
status: experimental
description: >
  Detects cosign attest from non-CI hosts.
...
detection:
  selection:
    event_type: 'attestation_created_outside_ci'
  condition: selection
falsepositives:
...
```json
{
  ...
  "source": "linux",
  "event_type": "attestation_created_outside_ci",
  "description": "Triggered rule: Attestation Created Outside CI",
  "severity": "critical"
}
```
...
**Sigma -- OIDC Issuer Mismatch in Attestation:**

```yaml
title: OIDC Issuer Mismatch in Attestation
id: wl-46-84a914db
status: experimental
description: >
  Detects attestation OIDC issuer not matching expected CI.
author: WeakLink Labs
date: 2026/04/04
logsource:
...
detection:
  selection:
    event_type: 'oidc_issuer_mismatch_in_attest'
  condition: selection
falsepositives:
  - Legitimate operational activities
level: critical
...
```json
{
  ...
  "source": "ci_cd",
  "event_type": "oidc_issuer_mismatch_in_attest",
  "description": "Triggered rule: OIDC Issuer Mismatch in Attestation",
  "severity": "critical"
}
```
