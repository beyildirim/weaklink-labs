...
**Splunk -- detect attestations signed outside CI:**

```spl
index=artifact_security sourcetype=cosign_verify
| where isnotnull(attestation_signer)
| where NOT attestation_signer LIKE "%github.com/actions%"
  AND NOT attestation_signer LIKE "%gitlab.com%"
| where NOT src_ip IN ("10.0.1.100", "10.0.1.101")
| table _time, image_digest, attestation_signer, src_ip, builder_id_claim
| rename attestation_signer AS "Actual Signer", builder_id_claim AS "Claimed Builder"
```

**KQL -- detect OIDC issuer mismatch:**

```kql
ArtifactAttestations_CL
| extend claimed_builder = tostring(parse_json(predicate_s).builder.id)
| extend oidc_issuer = tostring(certificate_oidc_issuer_s)
| where claimed_builder has "github.com/actions"
  and oidc_issuer != "https://token.actions.githubusercontent.com"
| project TimeGenerated, image_digest_s, claimed_builder, oidc_issuer, signer_identity_s
```
