...
An SBOM is a JSON file. Without cryptographic signing, anyone with write access can remove a vulnerable component, change a version number, or delete an entire dependency tree. The modified SBOM passes schema validation and compliance checks without raising an alert.
