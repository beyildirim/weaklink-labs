...
!!! checkpoint "Phase 2 Checkpoint"
    ...
    - Why did `docker pull` not reject the extra layer?
    - What would a cosign signature on the original manifest digest detect?
    - Could this attack work if the image was pinned by digest in the deployment?
