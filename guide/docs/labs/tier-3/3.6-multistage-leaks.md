...
!!! checkpoint "Phase 2 Checkpoint"
    ...
    - Which of the three leaks would `trivy image --scanners secret` catch?
    - Why does `ARG` in the builder stage still leak if the final stage re-declares it?
    - What is the difference between `ENV` and `ARG` in terms of persistence?
