...
!!! checkpoint "Phase 2 Checkpoint"
    ...
    - What Docker config setting determined which registry was checked first?
    - How does this differ from pip's `--extra-index-url` dependency confusion?
    - Would digest pinning alone prevent this if the digest references the wrong registry?
