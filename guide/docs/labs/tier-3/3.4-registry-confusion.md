...
**Suricata rule -- detect pulls from unapproved registries:**

```yaml
alert http $KUBE_NODES any -> !$APPROVED_REGISTRIES any (
  msg:"WEAKLINK - Container image pull from unapproved registry";
...
**Splunk -- detect image pulls from unapproved registries:**

```spl
index=kubernetes sourcetype=kube:events
  reason="Pulling"
| rex field=message "Pulling image \"(?<image>[^\"]+)\""
| where NOT match(image, "^registry:5000/")
  AND NOT match(image, "^gcr\.io/distroless/")
  AND NOT match(image, "^registry\.k8s\.io/")
| table _time, host, image, namespace
```

**Splunk -- detect unqualified image names in deployments:**

```spl
index=kubernetes sourcetype=kube:deployments
| spath output=containers path=spec.template.spec.containers{}
| mvexpand containers
| spath input=containers output=image path=image
| where NOT match(image, "^[a-z0-9.-]+[.:][a-z0-9]+/")
| table _time, name, namespace, image
```

**KQL -- detect pulls from unexpected registries:**

```kql
ContainerImageEvents
| where isnotempty(Image)
| extend Registry = extract(@"^([^/]+)/", 1, Image)
| where Registry !in ("registry:5000", "gcr.io", "registry.k8s.io")
| project TimeGenerated, PodName, Namespace, Image, Registry, Node
```

**KQL -- find deployments with unqualified image names:**

```kql
KubePodInventory
| where isnotempty(ContainerImage)
| where ContainerImage !contains "." and ContainerImage !contains ":"
| project TimeGenerated, PodName, Namespace, ContainerImage, ClusterName
```
