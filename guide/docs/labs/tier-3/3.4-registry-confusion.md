...
**Sigma -- Image Pulled from Unapproved Registry:**

```yaml
title: Image Pulled from Unapproved Registry
id: wl-34-9e26c8ef
status: experimental
description: >
  Detects container pulls from registries not on approved list.
...
detection:
  selection:
    event_type: 'image_pulled_from_unapproved_r'
  condition: selection
falsepositives:
...
```json
{
  ...
  "source": "kubernetes",
  "event_type": "image_pulled_from_unapproved_r",
  "description": "Triggered rule: Image Pulled from Unapproved Registry",
  "severity": "high"
}
```
...
**Sigma -- Unqualified Image Name in Deployment:**

```yaml
title: Unqualified Image Name in Deployment
id: wl-34-bdbe3785
status: experimental
description: >
  Detects unqualified image names vulnerable to registry confusion.
author: WeakLink Labs
date: 2026/04/04
logsource:
  category: application
...
detection:
  selection:
    event_type: 'unqualified_image_name_in_depl'
  condition: selection
falsepositives:
  - Legitimate operational activities
level: medium
...
```json
{
  ...
  "source": "kubernetes",
  "event_type": "unqualified_image_name_in_depl",
  "description": "Triggered rule: Unqualified Image Name in Deployment",
  "severity": "medium"
}
```
...
**Sigma -- DNS Resolution to Docker Hub from Production:**

```yaml
title: DNS Resolution to Docker Hub from Production
id: wl-34-15ef6e21
status: experimental
description: >
  Detects Docker Hub DNS queries from production nodes.
author: WeakLink Labs
date: 2026/04/04
logsource:
  category: dns
...
detection:
  selection:
    event_type: 'dns_resolution_to_docker_hub_f'
  condition: selection
falsepositives:
  - Legitimate operational activities
level: medium
...
```json
{
  ...
  "source": "dns_server",
  "event_type": "dns_resolution_to_docker_hub_f",
  "description": "Triggered rule: DNS Resolution to Docker Hub from Production",
  "severity": "medium"
}
```
