...
`docker pull` downloads a **stack of compressed tarballs** (layers), a **manifest** listing them in order, and a **config blob** with metadata. Attackers hide malicious content in layers that appear "deleted" in the final filesystem but remain extractable from the image.
...
!!! checkpoint "Phase 2 Checkpoint"
    ...
    - What is a whiteout file and what does it do?
    - Why does `docker run` not show the hidden file but `tar tf` on the layer does?
    - How many layers does `webapp:latest` have vs `webapp:clean`?
