...
**Splunk -- Detect secret exfiltration in CI logs:**

```spl
index=cicd sourcetype=build_log
| regex _raw="(SECRET|TOKEN|API_KEY|PASSWORD).*=.*[a-zA-Z0-9]{16,}"
| where NOT like(source, "%test%")
| stats count by repo, committer, _time
```

**KQL (Microsoft Sentinel) -- Detect direct pushes to main:**

```kql
GitAuditLogs
| where Action == "push" and TargetBranch == "main"
| where PushType != "merge_commit"
| project TimeGenerated, Actor, Repository, CommitCount
```
