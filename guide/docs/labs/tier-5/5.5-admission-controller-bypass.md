...
**Suricata rule -- detect traffic from kube-system pods to external networks:**

```yaml
alert ip $K8S_POD_CIDR any -> $EXTERNAL_NET any (
  msg:"WEAKLINK - Outbound traffic from kube-system pod";
...
**Splunk -- detect pod creation in exempt namespaces:**

```spl
index=k8s sourcetype=kube:apiserver:audit
  verb="create" objectRef.resource="pods"
  objectRef.namespace IN ("kube-system", "gatekeeper-system", "kyverno")
| where NOT match(user.username, "^system:")
| table _time, user.username, objectRef.name, objectRef.namespace, sourceIPs{}
```

**Splunk -- detect post-admission security context changes:**

```spl
index=k8s sourcetype=kube:apiserver:audit
  verb="patch"
  objectRef.resource IN ("deployments", "daemonsets", "statefulsets")
| spath output=patch_body path=requestObject
| where match(patch_body, "privileged.*true|hostNetwork.*true|hostPID.*true")
| table _time, user.username, objectRef.name, objectRef.namespace
```

**KQL (Microsoft Sentinel) -- admission webhook failures:**

```kql
KubeAuditLogs
| where ResponseStatus_code >= 500
| where UserAgent has "webhook"
| summarize FailureCount=count() by bin(TimeGenerated, 5m), ObjectRef_Resource
| where FailureCount > 10
| project TimeGenerated, ObjectRef_Resource, FailureCount
```

**KQL -- Gatekeeper audit violations (post-admission drift):**

```kql
KubeEvents
| where Source has "gatekeeper"
| where Reason == "AuditViolation"
| extend ViolatedConstraint = extract("constraint: (\\S+)", 1, Message)
| project TimeGenerated, InvolvedObject_Name, InvolvedObject_Namespace, ViolatedConstraint
```
