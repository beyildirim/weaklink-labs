...
**Suricata rule -- detect exfiltration from Terraform runs:**

```yaml
alert http $CI_RUNNERS any -> !$CLOUD_APIS any (
  msg:"WEAKLINK - Potential credential exfiltration from Terraform local-exec";
...
**Splunk -- detect terraform spawning network tools:**

```spl
index=edr sourcetype=crowdstrike:events
  grandparent_process_name="terraform"
  process_name IN ("curl", "wget", "nc", "ncat", "python", "python3", "ruby", "perl")
| table _time, host, grandparent_process_name, parent_process_name,
        parent_command_line, process_name, process_command_line
| sort -_time
```

**Splunk -- detect HTTP POST from CI during Terraform run:**

```spl
index=proxy sourcetype=squid
  http_method="POST"
  src_ip IN ("10.200.0.0/24")
  NOT dest_host IN ("*.amazonaws.com", "*.googleapis.com", "*.azure.com",
                     "registry.terraform.io", "releases.hashicorp.com")
| where match(_raw, "terraform") OR match(user_agent, "curl|wget|python")
| table _time, src_ip, dest_host, dest_port, uri_path, user_agent
```

**KQL (Microsoft Sentinel) -- terraform child process executing network tools:**

```kql
DeviceProcessEvents
| where InitiatingProcessFileName == "terraform" or
        InitiatingProcessParentFileName == "terraform"
| where FileName in ("curl", "wget", "nc", "python", "python3", "sh", "bash")
| where ProcessCommandLine has_any ("http://", "https://", "/dev/tcp",
                                      "AWS_ACCESS_KEY", "AWS_SECRET")
| project Timestamp, DeviceName, FileName, ProcessCommandLine,
          InitiatingProcessCommandLine
```

**KQL -- null_resource in Terraform state (periodic audit):**

```kql
DeviceFileEvents
| where FileName == "terraform.tfstate"
| where FileOperation == "write"
| extend StateContent = parse_json(FileContent)
| where StateContent has "null_resource"
