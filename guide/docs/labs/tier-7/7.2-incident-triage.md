...
You are the on-call SOC analyst. The alert fired on a rule you wrote in Lab 7.1. Three hours have passed since the malicious package was installed. Every minute you spend investigating is a minute the attacker has to deepen their access.
...
index=proxy sourcetype=squid
  dest_host="pypi.org"
  uri_path="*internal-utils*"
  uri_path="*99.0.0*"
| stats earliest(_time) AS first_seen, latest(_time) AS last_seen, count by src_ip
| lookup asset_inventory src_ip OUTPUT hostname, team, environment
| sort first_seen
```
