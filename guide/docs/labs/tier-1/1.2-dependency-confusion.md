...
<div class="lab-meta">
  ...
  <span>Prerequisites: Lab 1.1</span>
</div>
...
**Suricata rule -- detect setup.py exfiltration via HTTP POST:**

```yaml
alert http $BUILD_SERVERS any -> $EXTERNAL_NET any (
  msg:"WEAKLINK - Potential dependency confusion exfiltration via HTTP POST during pip install";
...
**Suricata rule -- detect DNS exfiltration from build servers:**

```yaml
alert dns $BUILD_SERVERS any -> any 53 (
  msg:"WEAKLINK - Potential DNS exfiltration from build server (long subdomain)";
  dns.query;
  pcre:"/^[a-zA-Z0-9]{30,}\./";
  classtype:trojan-activity;
  sid:2100003; rev:1;
...
**Splunk -- detect pip installing from public PyPI when internal package name is requested:**
...
| table _time, src_ip, package_name, uri_path, status
```

**Splunk -- detect setup.py spawning child processes (exfiltration):**
...
**Splunk -- detect unexpected outbound connections during pip install:**

```spl
index=network sourcetype=firewall action=allowed
  src_ip IN ("10.100.0.0/24")
  dest_port IN (80, 443, 8080, 4443)
  dest_ip!=10.0.0.0/8
  dest_ip!=172.16.0.0/12
  dest_ip!=192.168.0.0/16
| where NOT match(dest_host, "(pypi\.internal\.corp|internal-pypi)")
| bin _time span=1m
| stats values(dest_host) AS external_hosts, dc(dest_host) AS unique_hosts by src_ip, _time
| where unique_hosts > 2
```

**KQL (Microsoft Sentinel) -- setup.py process tree anomaly:**
...
| join kind=inner (
    DeviceProcessEvents
    | where ProcessCommandLine has_any ("curl", "wget", "/tmp/", "environ",
                                         "whoami", "hostname")
) on DeviceId, $left.ProcessId == $right.InitiatingProcessParentId
| project Timestamp, DeviceName, SetupPyCmd=ProcessCommandLine,
          SuspiciousChild=ProcessCommandLine1
```

**KQL -- high-version package install (version > 50 is suspicious):**
