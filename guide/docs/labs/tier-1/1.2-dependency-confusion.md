...
`acme-auth` version 99.0.0 on the public registry. The attacker found the internal package name (from a leaked `requirements.txt`, a job posting, or a public repo) and published a higher version.
...
**Checkpoint:** You should now have `acme-auth==99.0.0` installed, `/tmp/dependency-confusion-pwned` present, and the app reporting compromise.
...
The difference: `index-url` (without `extra-`) makes pip search **only** the specified registry. The public registry is gone. Even if the attacker publishes `acme-auth==99.0.0` on public PyPI, pip will never see it.
...
- pip output showing a version jump (e.g., `acme-auth` from `1.0.0` to `99.0.0`)
- pip downloading from `pypi.org` a package matching your internal namespace (`acme-*`, `company-*`)
...
??? tip "SOC Relevance"
    ...
    **Triage workflow:**
    ...
    1. **Package name**: matches an internal namespace (`acme-*`, `company-*`)? Escalate immediately.
