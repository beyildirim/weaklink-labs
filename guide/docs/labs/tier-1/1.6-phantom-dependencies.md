...
**Splunk -- High-version package installs (substitution indicator):**

```splunk
index=ci sourcetype=build_log command="npm install" OR command="npm update"
| rex field=_raw "added [\w@-]+@(?<version>\d+)\.\d+\.\d+"
| where version > 50
| table _time, build_id, project, package_name, version
```

**Splunk -- Package appears in node_modules but not in package.json:**

```splunk
index=ci sourcetype=depcheck_audit
| where status="missing_dependency"
| stats count by project, package_name, imported_in_file
| sort - count
| table project, package_name, imported_in_file, count
```

**Splunk -- npm update introduces unexpected new packages:**

```splunk
index=ci sourcetype=build_log command="npm update"
| rex field=_raw "added (?<new_packages>\d+) packages"
| where new_packages > 0
| rex field=_raw "added (?<pkg_list>[\w@\-./\s,]+)"
| table _time, build_id, project, new_packages, pkg_list
```

**KQL (Microsoft Sentinel) -- Version jump anomaly detection:**

```kql
NpmInstallLogs
| extend PackageName = extract(@"([\w\-@/]+)@(\d+\.\d+\.\d+)", 1, InstalledPackage)
| extend Version = extract(@"@(\d+)\.\d+\.\d+", 1, InstalledPackage)
| where toint(Version) > 50
| project TimeGenerated, BuildId, PackageName, InstalledPackage, Project
```

**KQL -- New packages appearing after npm update:**

```kql
DeviceFileEvents
| where ActionType == "FileCreated"
| where FolderPath has "node_modules" and FileName == "package.json"
| extend PackageName = extract(@"node_modules[/\\](@?[^/\\]+(?:[/\\][^/\\]+)?)[/\\]package.json", 1, FolderPath)
| where PackageName !in (
    // Your declared dependencies -- populate from package.json
    "acme-framework", "express", "lodash"
)
| project Timestamp, DeviceName, PackageName, FolderPath
```
