...
**Splunk -- Git audit log (lockfile-only PRs from non-bot accounts):**

```splunk
index=github sourcetype=audit_log action=pull_request
| eval files=mvindex(files_changed, 0, -1)
| where like(files, "%lock%") OR like(files, "%requirements.txt%")
| where NOT like(files, "%requirements.in%") AND NOT like(files, "%package.json%") AND NOT like(files, "%Pipfile%")
| where NOT author IN ("dependabot[bot]", "renovate[bot]", "github-actions[bot]")
| table _time, repo, author, pr_number, pr_title, files
```

**Splunk -- Unexpected network connections during pip install:**

```splunk
index=ci sourcetype=build_log process="pip" OR process="pip3"
| rex field=_raw "Downloading (?<download_url>https?://[^\s]+)"
| where NOT like(download_url, "%pypi.org%") AND NOT like(download_url, "%pypi-private%")
| stats count by build_id, download_url, project
```

**KQL (Microsoft Sentinel) -- Lockfile-only PRs:**

```kql
GitHubAuditLog
| where Action == "pull_request.opened" or Action == "pull_request.synchronize"
| mv-expand FilesChanged
| where FilesChanged has_any ("lock", "requirements.txt")
| where FilesChanged !has "requirements.in" and FilesChanged !has "package.json"
| where Actor !in ("dependabot[bot]", "renovate[bot]")
| project TimeGenerated, Repository, Actor, PullRequestNumber, FilesChanged
```

**KQL -- Suspicious child processes during package install:**

```kql
DeviceProcessEvents
| where InitiatingProcessFileName in ("pip", "pip3", "npm", "node")
| where FileName in ("bash", "sh", "curl", "wget", "powershell", "cmd.exe")
