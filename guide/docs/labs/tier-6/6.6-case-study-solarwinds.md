...
**Suricata rule -- detect SUNBURST DNS C2:**

```yaml
alert dns any any -> any any (
  msg:"WEAKLINK - SUNBURST C2 DNS query to avsvmcloud.com";
...
**Splunk -- detect SUNBURST DNS beaconing:**
...
**Splunk -- detect anomalous DLL in SolarWinds process:**

```spl
index=edr sourcetype=sysmon EventCode=7
  process_name="SolarWinds.BusinessLayerHost.exe"
| where NOT match(ImageLoaded, "SolarWinds|Microsoft|System")
| table _time, host, process_name, ImageLoaded, Hashes
```

**Splunk -- detect lateral movement from SolarWinds servers:**

```spl
index=windows sourcetype=WinEventLog:Security EventCode=4624
  LogonType=3 OR LogonType=10
| where src_ip IN (solarwinds_server_ips)
| where dest NOT IN (solarwinds_server_ips)
| table _time, src_ip, dest, user, LogonType
```

**KQL (Microsoft Sentinel) -- SUNBURST DNS indicators:**
...
**KQL -- anomalous outbound connections from SolarWinds servers:**

```kql
DeviceNetworkEvents
| where DeviceName has "solarwinds" or InitiatingProcessFileName has "SolarWinds"
| where RemoteUrl !has "solarwinds.com"
| where RemotePort in (80, 443)
| summarize Connections=count() by RemoteUrl, RemoteIP
| where Connections < 10
| project RemoteUrl, RemoteIP, Connections
```

**KQL -- build artifact hash mismatch (generic detection):**

```kql
DeviceFileEvents
| where FileName endswith ".dll" or FileName endswith ".exe"
| where FolderPath has "SolarWinds"
| extend FileHash = SHA256
| join kind=anti (
    externaldata(ExpectedHash:string) [@"https://ti.example.com/solarwinds-known-good-hashes.csv"]
) on $left.FileHash == $right.ExpectedHash
