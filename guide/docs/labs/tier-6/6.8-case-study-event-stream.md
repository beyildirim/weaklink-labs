...
**Suricata rule -- detect cryptocurrency mining from development machines:**

```yaml
alert tls $DEV_MACHINES any -> $EXTERNAL_NET any (
  msg:"WEAKLINK - Crypto mining pool connection from dev machine";
...
**Splunk -- detect install script abuse:**

```spl
index=edr sourcetype=crowdstrike:events
  (parent_process_name="node" OR parent_process_name="npm")
  process_name IN ("curl", "wget", "powershell", "cmd.exe", "sh")
| where match(process_command_line, "(http|ftp|\.exe|\.sh)")
| table _time, host, parent_process_name, process_name, process_command_line
```

**Splunk -- detect maintainer changes in npm registry:**

```spl
index=npm_registry sourcetype=registry_audit
  action="maintainer_added" OR action="maintainer_removed"
| where package IN (critical_package_list)
| table _time, package, action, maintainer, changed_by
```

**KQL (Microsoft Sentinel) -- crypto miner launched by npm:**

```kql
DeviceProcessEvents
| where InitiatingProcessFileName in ("node", "node.exe", "npm", "npm.cmd")
| where FileName has_any ("miner", "xmrig", "cpuminer")
    or ProcessCommandLine has_any ("stratum", "pool", "mining")
| project Timestamp, DeviceName, FileName, ProcessCommandLine, InitiatingProcessCommandLine
```

**KQL -- new dependency detection in lockfile changes:**
