...
**Splunk -- cross-layer kill chain detection:**

```spl
index=cicd sourcetype=github_audit action="package_added"
| join pr_number [
    search index=cicd sourcetype=github_audit action="workflow_modified"
]
| join commit_sha [
    search index=registry sourcetype=docker_push
    | where image_layers_changed=true
]
| table _time, pr_number, package_name, workflow_file, image_tag, commit_sha
| sort _time
```

**Splunk -- new dependency + workflow change in same PR:**

```spl
index=cicd sourcetype=github_pr
| transaction pr_number
| where match(files_changed, "package.json") AND match(files_changed, ".github/workflows/")
| table _time, pr_number, author, files_changed
```

**KQL (Microsoft Sentinel) -- multi-stage supply chain correlation:**

```kql
let PackageEvents = DeviceProcessEvents
| where ProcessCommandLine has "npm install"
| where Timestamp > ago(7d)
| project PackageTime=Timestamp, DeviceName, ProcessCommandLine;
let CIEvents = AzureDevOpsAuditing
| where OperationName == "Pipeline.Modified"
| project CITime=Timestamp, PipelineName, ModifiedBy;
let ImageEvents = ContainerRegistryEvents
| where OperationName == "Push"
| project PushTime=Timestamp, Repository, Digest;
PackageEvents
| join kind=inner CIEvents on $left.DeviceName == $right.ModifiedBy
| where CITime between (PackageTime .. PackageTime + 24h)
| join kind=inner ImageEvents on $left.PipelineName == $right.Repository
| where PushTime between (CITime .. CITime + 2h)
| project PackageTime, CITime, PushTime, ProcessCommandLine, PipelineName, Digest
```

**KQL -- production pod connecting to unknown endpoint after image update:**

```kql
ContainerNetworkLogs
| where Direction == "outbound"
| where DestinationIP !in (approved_endpoints)
| join kind=inner (
    KubeEvents
    | where Reason == "Pulled"
    | where Timestamp > ago(1h)
) on PodName
| project Timestamp, PodName, DestinationIP, DestinationPort, ImageName
```
