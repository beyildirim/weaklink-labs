...
`ignore-scripts` prevents install-time code execution but breaks packages that need native compilation (esbuild, sharp, node-pre-gyp). Use a scoped approach: run `npm install` in a sandboxed environment first, audit the scripts with `npm pack` + manual review, then install in production. Alternatively, use `npm audit signatures` to verify package provenance.
