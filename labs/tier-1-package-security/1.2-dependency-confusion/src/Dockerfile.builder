...
RUN mkdir -p /build/output/private /build/output/public \
 && cd /build/packages/acme-auth-1.0.0 && python setup.py sdist bdist_wheel -q && cp dist/* /build/output/private/ \
 && cd /build/packages/acme-auth-99.0.0 && python setup.py sdist -q && cp dist/* /build/output/public/
