#!/bin/bash
# Build wl-auth packages for both registries.
...
echo "=== Building legitimate wl-auth 1.0.0 for private registry ==="
cd "${PACKAGES_DIR}/wl-auth-1.0.0"
python setup.py sdist bdist_wheel -q
cp dist/* "$PRIVATE_PACKAGES/"

echo "=== Building malicious wl-auth 99.0.0 for public registry ==="
cd "${PACKAGES_DIR}/wl-auth-99.0.0"
