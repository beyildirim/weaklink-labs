...
# Check 1: Registry is running (K8s service DNS)
...
HAS_DIGEST_PIN=$(grep -c '@sha256:' /workspace/Dockerfile.defended 2>/dev/null || echo 0)
...
USES_LATEST=$(grep -c ':latest' /workspace/Dockerfile.defended 2>/dev/null || echo 0)
...
SAFE_DIGEST=$(cat /workspace/safe-digest.txt 2>/dev/null | tr -d '[:space:]')
if [ -n "${SAFE_DIGEST}" ]; then
    DIGEST_IN_FILE=$(grep -c "${SAFE_DIGEST}" /workspace/Dockerfile.defended 2>/dev/null || echo 0)
...
IMAGE_EXISTS=$(docker images -q my-defended-app:v1 2>/dev/null | head -1)
